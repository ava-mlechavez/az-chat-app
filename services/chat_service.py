...
class ChatService:
    def chat(self: "ChatService", prompt: str, chat_history: list) -> str:
        ...
        content = openai_service.chat(messages=[
            ChatCompletionSystemMessageParam(
                role="system",
    ...
    def __create_chat_with_context(self: "ChatService", prompt: str, chat_history: list[ChatCompletionMessageParam], context: list) -> str:
        system_message: str = os.getenv(
            ...
            f"""
                ...
                - **Do not answer any questions that are not related to hotels. If the question is not about hotels, politely inform the user that you can only assist with hotel-related inquiries.**
            """,
        )
        return f"""
            {system_message}
